...
def create_scheduler(
    ...
    if warmup_epochs > 0:
        if warmup_factor == 0 and lr_epoch_stair:
            print("[WARNING]: The warmup factor is set to 0, lr of 0-th epoch is always zero! "
                  "Recommend value is 0.01.")
        warmup_func = linear_lr if lr_epoch_stair else linear_refined_lr
        warmup_lr_scheduler = warmup_func(start_factor=warmup_factor, end_factor=1.0, total_iters=warmup_epochs,
                                          lr=lr, steps_per_epoch=steps_per_epoch, epochs=warmup_epochs)
    ...
    if scheduler in ['cosine_decay', 'warmup_cosine_decay']:
        ...
        main_lr_scheduler = exponential_func(gamma=decay_rate,
                                             lr=lr, steps_per_epoch=steps_per_epoch, epochs=main_epochs)
    elif scheduler == 'polynomial_decay':
        polynomial_func = polynomial_lr if lr_epoch_stair else polynomial_refined_lr
        main_lr_scheduler = polynomial_func(total_iters=main_epochs, power=decay_rate,
                                            lr=lr, steps_per_epoch=steps_per_epoch, epochs=main_epochs)
    elif scheduler == 'step_decay':
        main_lr_scheduler = step_lr(step_size=decay_epochs, gamma=decay_rate,
                                    lr=lr, steps_per_epoch=steps_per_epoch, epochs=main_epochs)
    elif scheduler == 'multi_step_decay':
        main_lr_scheduler = multi_step_lr(milestones=milestones, gamma=decay_rate,
                                          lr=lr, steps_per_epoch=steps_per_epoch, epochs=main_epochs)
